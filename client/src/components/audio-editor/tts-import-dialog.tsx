...
**Reasoning:**
The user explicitly stated the intention to remove dialog parsing functionality and revert to a simpler text import. This involves several steps:
1.  **Remove `isDialogMode` state:** The `useState` hook for `isDialogMode` is removed as it's no longer needed.
2.  **Remove `parseDialogText` function:** This entire function was dedicated to parsing the dialog format and is removed.
3.  **Simplify `handleTextChange`:** The conditional logic that checked for `isDialogMode` and called `parseDialogText` is removed. The function now solely focuses on parsing the text into fragments using `TTSService.parseTextToFragments`.
4.  **Remove Dialog Mode JSX:** The `Checkbox` for "Tryb dialogu" and the associated `Card` for "Przypisz głosy do mówców" are removed from the JSX.
5.  **Remove `detectedSpeakers` state and related logic:** Since dialog mode is removed, the state for `detectedSpeakers` and any logic that populates or uses it is also removed.
6.  **Remove `speakerVoiceMapping` state and related logic:** This state was specific to mapping speakers in dialog mode and is removed.
7.  **Update `handleImport`:** The reset logic for `isDialogMode` and `speakerVoiceMapping` is removed as these states no longer exist.

These changes effectively remove the dialog parsing feature and restore the component to a state where it only handles simple text-to-audio fragment conversion with alternating voice support.
